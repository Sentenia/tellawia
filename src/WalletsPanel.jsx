...
export default function WalletsPanel({ wallets, setWallets }){
  ...
  const add = () => {
    ...
    setWallets(prev => [...prev, { address, label: label||'My Wallet', chain }])
    setAddress(''); setLabel('')
  }
          ...
          {wallets.map((w,i)=>(
            <div key={i} className="row">
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <span className="pill">{w.chain==='pulse'?'PulseChain':'Ethereum'}</span>
                <strong>{w.label}</strong>
              </div>
              <code style={{opacity:.85}}>{w.address}</code>
              <button className="btn" onClick={()=>remove(i)}>Remove</button>
            </div>
          ))}
        </div>
      )}
      {wallets.length===0 && <div className="muted" style={{marginTop:8}}>Add one or more wallet addresses to begin.</div>}
    </div>
  )
}
